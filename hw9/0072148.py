...
plt.ylim((-8,+8))
for i in range(N):
    plt.plot(X[(B == 1)[i,:]][:,0], X[(B == 1)[i,:]][:,1], color = "#C0C0C0", zorder = 0)
plt.plot(X[:,0], X[:,1], markersize = 5, marker = "o", linestyle = "none" ,color = "black", zorder = 10)
plt.show()
...
print("D: ")
print(D)
D_minus_sqrt = np.sqrt(sci.linalg.cho_solve(sci.linalg.cho_factor(D), np.eye(N)))
...
Z = selected_vectors
#Z = np.hstack([selected_vectors[i] for i in range (5)])
#print(Z[0:5, 0:5])
#print(Z.shape)
## K - means Clustering
...
for c in range(K):
    ...
    plt.plot(np.mean(X[memberships == c,0]), np.mean(X[memberships == c,1]), marker = "s", markersize = 7, markerfacecolor = point_colors[c], markeredgecolor = "black")
plt.show()
