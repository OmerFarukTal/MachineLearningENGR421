...
confusion_matrix = pd.crosstab(class_assignment_points + 1, classes + 1, rownames = ['y_truth'], colnames = ['y_pred'])

print(confusion_matrix)

### Scoring functions

scoring_W = np.stack([-0.5*np.linalg.inv(sample_covariances[l]) for l in range(4)])

scoring_w = np.stack([np.dot(np.linalg.inv(sample_covariances[l]), sample_means[l]).T for l in range(4)])

scoring_w0 = np.stack([-0.5*np.dot(np.dot(sample_means[l].T, np.linalg.inv(sample_covariances[l])), sample_means[l]) 
                       -np.log(2*math.pi) 
                       -0.5*np.log(np.linalg.det(sample_covariances[l]))
                       +np.log(class_priors[l]) for l in range(4)])

scoring = np.array([[ np.dot(np.dot(data_interval[k,:], scoring_W[l]), data_interval[k,:].reshape(2,1)) 
                    + np.dot(scoring_w[l], data_interval[k,:].reshape(2,1))
                    + scoring_w0[l]
